...
(async () => {
  ...
  `;
    ...
    /* ---------------- import an existing pack for editing ---------------- */

    // Rebuilds one tracks[] entry from a manifest record + the matching .ogg
    // pulled out of the uploaded zip. Colors come straight from the manifest
    // (buildTexture() regenerates the exact same disc art from them); audio
    // is reused as-is rather than re-encoded.
    async function importTrackFromManifest(zip, namespace, packName, manifestTrack) {
        const oggPath = `resourcepack/${packName}/assets/${namespace}/sounds/records/${manifestTrack.id}.ogg`;
        const oggEntry = zip.file(oggPath);
        if (!oggEntry) throw new Error(`missing audio at ${oggPath}`);
        if (!manifestTrack.colors || !manifestTrack.colors.main || !manifestTrack.colors.ring) {
            throw new Error("manifest entry is missing color data");
        }

        const oggBytes = await oggEntry.async("uint8array");
        const oggBlob = new Blob([oggBytes], { type: "audio/ogg" });

        if (!TEMPLATE) await loadTemplate();

        const title = manifestTrack.title || "Unknown Title";
        const track = {
            id: manifestTrack.id || slugify(title),
            // a File wrapper around the extracted ogg, so every place that
            // reads track.file.name (renderTrack, logging, etc.) keeps working
            // without needing to special-case imported tracks
            file: new File([oggBlob], title + ".ogg", { type: "audio/ogg" }),
            title,
            artist: manifestTrack.artist || "Unknown Artist",
            cover: null,
            colors: manifestTrack.colors,
            canvas: null,
            oggBlob: null,
            duration: manifestTrack.duration || 180,
            status: "ready",
            error: null,
            importedOggBytes: oggBytes
        };
        track.canvas = buildTexture(track.colors.main, track.colors.ring);
        return track;
    }
    ...
    async function importPack(file) {
        log(`Reading uploaded pack "${file.name}"...`);
        let zip;
        try {
            zip = await JSZip.loadAsync(file);
        } catch (e) {
            log(`Could not open "${file.name}" as a zip: ${e.message}`, "err");
            return;
        }

        const manifestEntry = zip.file("discpress-manifest.json");
        if (!manifestEntry) {
            log("No discpress-manifest.json found in this zip - it either wasn't generated by Disc Press, or predates this feature. Can't auto-import; add tracks manually instead.", "err");
            return;
        }
        ...
        try {
            ...
            log("discpress-manifest.json is corrupt: " + e.message, "err");
            return;
        }

        if (manifest.namespace) $("#namespace").value = manifest.namespace;
        if (manifest.packName) $("#packname").value = manifest.packName;

        let imported = 0, failed = 0;
        for (const mt of manifest.tracks || []) {
            try {
                const track = await importTrackFromManifest(zip, manifest.namespace, manifest.packName, mt);
                if (usedIds.has(track.id)) {
                    const oldId = track.id;
                    track.id = uniqueId(track.id, usedIds);
                    log(`  note: "${track.title}" id collided with an existing track, renamed ${oldId} -> ${track.id}`);
                } else {
                ...
                failed++;
                log(`  skipped "${mt.title || mt.id}": ${e.message}`, "err");
            }
        }
        refreshEmpty();
        log(`Imported ${imported} track(s) from "${file.name}"` + (failed ? `, ${failed} failed` : "") + ". Edit, add, or remove tracks, then generate again.", imported ? "ok" : "err");
    }
    ...
    async function convertToOgg(track) {
        // Tracks pulled in from an uploaded pack (see importPack()) already
        // carry pre-encoded OGG Vorbis bytes and a known duration lifted
        // straight from the old zip, so there's no need to run them through
        // ffmpeg again. This only applies until the track's audio is
        // explicitly replaced with a new file.
        if (track.importedOggBytes) {
            return new Blob([track.importedOggBytes], { type: "audio/ogg" });
        }
    ...
    async function generateAll() {
        ...
        // ---- manifest, so this zip can be re-uploaded and edited later ----
        // Colors and title/artist/duration are the only things that can't be
        // cleanly recovered from the baked textures/sounds alone, so they're
        // the only things stored here. Audio and disc art are rebuilt from
        // the .ogg files and buildTexture() respectively on import.
        zip.file("discpress-manifest.json", JSON.stringify({
            format: 1,
            generator: "Disc Press",
            namespace,
            packName,
            tracks: tracks.map(t => ({
                id: t.id,
